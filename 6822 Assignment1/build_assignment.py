...
def deck_problem_page(pdf, metrics):
    ...
    cards = [
        (0.055, "Input", "Trained logistic score", "predict_proba becomes model_score", "#e7f5ff"),
        (0.375, "US lens", f"{us['threshold']:.2f} warning level", "Adverse-action reasons + monitoring", "#e6fcf5"),
        (0.695, "EU lens", f"{eu['threshold']:.2f} internal escalation", "Article 10/14/17 evidence package", "#fff4e6"),
    ]
    for x, label, headline, body, color in cards:
        ax.add_patch(plt.Rectangle((x, 0.55), 0.25, 0.20, transform=ax.transAxes, facecolor=color, edgecolor="#495057", linewidth=1.1))
        ax.text(x + 0.02, 0.715, label.upper(), transform=ax.transAxes, fontsize=8.5, fontweight="bold", color="#52616b", va="center")
        ax.text(x + 0.02, 0.655, headline, transform=ax.transAxes, fontsize=12, fontweight="bold", color="#17324d", va="center")
        ax.text(x + 0.02, 0.595, wrap_cell(body, 29), transform=ax.transAxes, fontsize=9.2, color="#1f2933", va="center", linespacing=1.25)
    for start in [0.305, 0.625]:
        ax.annotate("", xy=(start + 0.055, 0.65), xytext=(start, 0.65), xycoords=ax.transAxes, arrowprops=dict(arrowstyle="->", lw=1.6, color="#6b7280"))

    ax.add_patch(plt.Rectangle((0.055, 0.25), 0.89, 0.17, transform=ax.transAxes, facecolor="#f8f9fa", edgecolor="#ced4da", linewidth=1.0))
    ...
    for i, bullet in enumerate(bullets):
        ax.text(0.095, 0.325 - i * 0.045, f"- {bullet}", transform=ax.transAxes, fontsize=9.6, color="#1f2933", va="center")
...
def deck_citi_meridian_page(pdf):
    ...
    for x, title, color, bullets in columns:
        ...
        for i, bullet in enumerate(bullets):
            ax.text(x + 0.05, 0.58 - i * 0.095, f"- {bullet}", transform=ax.transAxes, fontsize=10.2, color="#1f2933", va="center")
    ax.add_patch(plt.Rectangle((0.18, 0.15), 0.64, 0.08, transform=ax.transAxes, facecolor="#fff9db", edgecolor="#ffe066", linewidth=1.0))
    ax.text(0.50, 0.19, "Boundary: this is a plausible design case, not proof of Citi's actual model inventory or compliance posture.", transform=ax.transAxes, ha="center", va="center", fontsize=9.5, color="#1f2933")
    ax.text(0.04, 0.06, f"{STUDENT['name']} | {STUDENT['matriculation_id']} | Senior management deck", transform=ax.transAxes, fontsize=8.5, color="#6b7280")
    pdf.savefig(fig, bbox_inches="tight")
    plt.close(fig)
...
def deck_meridian_value_page(pdf):
    ...
    ax.text(0.04, 0.855, "The product is valuable because it converts regulatory divergence into repeatable evidence, not because it promises legal certainty.", transform=ax.transAxes, fontsize=11.5, color="#52616b", va="top")
    rows = [
    ...
    for i, (title, body, color) in enumerate(rows):
        ...
        ax.text(0.39, y + 0.048, body, transform=ax.transAxes, fontsize=10, color="#1f2933", va="center")
    ax.text(0.08, 0.12, "Commercial logic: sell a narrow, auditable product to global banks that must monitor model and rule changes continuously.", transform=ax.transAxes, fontsize=10, fontweight="bold", color="#0b7285")
    ax.text(0.04, 0.06, f"{STUDENT['name']} | {STUDENT['matriculation_id']} | Senior management deck", transform=ax.transAxes, fontsize=8.5, color="#6b7280")
    pdf.savefig(fig, bbox_inches="tight")
    plt.close(fig)
...
def deck_architecture_page(pdf):
    ...
    for x, y, label, color in boxes:
        ...
        ax.text(x + 0.09, y + 0.07, label, transform=ax.transAxes, ha="center", va="center", fontsize=10, fontweight="bold", color="#1f2933")
    arrows = [
        ((0.23, 0.69), (0.27, 0.69)),
        ((0.45, 0.69), (0.49, 0.69)),
        ((0.67, 0.69), (0.71, 0.69)),
    ...
    ax.text(0.05, 0.13, "Design principle: the tool calculates, records and escalates; it does not make the final credit-policy or legal-compliance judgement.", transform=ax.transAxes, fontsize=10, color="#1f2933")
    pdf.savefig(fig, bbox_inches="tight")
    plt.close(fig)
...
def deck_sample_output_page(pdf, outputs):
    ...
    display = rejected[["applicant_id", "model_score", "decision_reason_1", "decision_reason_2"]]
    table = ax.table(cellText=display.values, colLabels=display.columns, cellLoc="left", colLoc="left", bbox=[0.04, 0.38, 0.92, 0.36])
    table.auto_set_font_size(False)
    table.set_fontsize(8.5)
    for (row, col), cell in table.get_celld().items():
        cell.set_edgecolor("#adb5bd")
...
def deck_boundary_choice_page(pdf):
    ...
    for (title, body), (x, y), color in zip(boundaries, positions, colors):
        ax.add_patch(plt.Rectangle((x, y), 0.38, 0.17, transform=ax.transAxes, facecolor=color, edgecolor="#495057", linewidth=1.1))
        ax.text(x + 0.03, y + 0.125, title, transform=ax.transAxes, fontsize=11.5, fontweight="bold", color="#17324d", va="center")
        ax.text(x + 0.03, y + 0.060, body, transform=ax.transAxes, fontsize=8.9, color="#1f2933", va="center", linespacing=1.35)
    ax.text(0.50, 0.13, "Management message: the tool escalates evidence to humans; it does not hide policy choices inside automation.", transform=ax.transAxes, ha="center", fontsize=10, color="#1f2933")
    pdf.savefig(fig, bbox_inches="tight")
    plt.close(fig)
